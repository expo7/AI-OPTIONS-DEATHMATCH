...
- Exact commit deployment, internal and public health checks; no secrets in Git. The initial release was manually bootstrapped; CI deployment automation is still outstanding.

See `V1_LAUNCH_PLAN.md` for the full V1 contract. Current next action: implement bot accounting and idempotent broker event attribution, then provision dedicated paper credentials and test same-contract conflicts. Public results must continue to disclose that no competition trades have begun until the experiment actually runs.
